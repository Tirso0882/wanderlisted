"""Run Activities Layer 1 evaluators against live, cached trajectories."""
...
from edd.activities.l1_evaluate import EVALUATORS  # noqa: E402
from edd.activities.run_utils import (  # noqa: E402
    BASELINE_CONFIG,
    classify_activities_outcome,
    run_activities_dataset,
)
...
async def main() -> None:
    print(
        f"\nActivities agent - Layer 1 over {len(DATASET)} cases"
        f"   |   model: {AGENT} {MODELS[AGENT]}"
    )
    print("=" * 74)

    queries = [case["query"] for case in DATASET]
    trajectories = await run_activities_dataset(queries, model_config=MODELS[AGENT])
    ...
    evaluated = 0
    perfect = 0
    outcomes = [classify_activities_outcome(trajectory) for trajectory in trajectories]
