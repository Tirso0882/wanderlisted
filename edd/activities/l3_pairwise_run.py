"""Run an Activities Layer 3 round-robin model tournament."""
...
from edd.activities.l1_dataset import DATASET  # noqa: E402
from edd.activities.l3_pairwise import (  # noqa: E402
    build_pairwise_judge,
    judge_pairwise,
)
from edd.activities.run_utils import BASELINE_CONFIG, run_activities_dataset  # noqa: E402
from edd.models import MODELS  # noqa: E402

LINEUP = ["terra", "luna"]
JUDGE = "sol"
# Each battle makes two order-swapped judge calls. Two concurrent battles fill
# the reasoning tier's four slots without queueing inside the timeout.
...
async def main() -> None:
    ...
    print(
        f"\nActivities agent - Layer 3 over {len(DATASET)} cases"
    ...
    for name in LINEUP:
        trajectories[name] = await run_activities_dataset(
    ...
    print("\n" + "=" * 74)
    print("LEADERBOARD - win rate (ties = 1/2), best activities model first:")
    ...
    if flip_details:
        ...
        for detail in flip_details:
            ...
            print(
                f"  forward -> {detail['winner_forward']}: "
                f"{detail['reasoning_forward']}"
            )
            print(
                f"  reverse -> {detail['winner_reverse']}: "
