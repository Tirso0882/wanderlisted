"""Run readiness Layer 1 evaluators against live, cached trajectories."""
...
from edd.baseline_store import record_component_report  # noqa: E402
from edd.readiness.l1_dataset import DATASET  # noqa: E402
from edd.readiness.l1_evaluate import EVALUATORS  # noqa: E402
from edd.readiness.run_utils import (  # noqa: E402
    BASELINE_CONFIG,
    classify_readiness_outcome,
    run_readiness_dataset,
)
...
async def main() -> None:
    print(
        f"\nReadiness agent - Layer 1 over {len(DATASET)} cases"
        f"   |   synthesis model: {AGENT} {MODELS[AGENT]}"
        "   |   planner: deterministic"
    )
    print("=" * 74)

    queries = [case["query"] for case in DATASET]
    trajectories = await run_readiness_dataset(queries, model_config=MODELS[AGENT])
    ...
    evaluated = 0
    perfect = 0
    outcomes = [classify_readiness_outcome(item) for item in trajectories]
    ...
    record_component_report(
        ...
        context={"planner_model": "deterministic readiness planner"},
        report_source_files=(
            Path(__file__),
