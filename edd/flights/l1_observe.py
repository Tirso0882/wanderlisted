"""EDD — observe the FLIGHTS agent, this time with LangSmith tracing.

Flights is our first WORKER agent: it calls a LIVE external API (Duffel). So
...
async def main() -> None:
    query = (
        "Find round-trip flights from New York to Tokyo, departing 2026-08-15, "
        "returning 2026-08-22, for 1 adult in economy."
    )
    ...
    with collect_runs() as runs_cb:
        traj = await run_agent(FlightsAgent, query)
