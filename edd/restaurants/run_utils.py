"""Bounded live-run and trajectory-cache helpers for Restaurant EDD."""
...
from src.agent.agents import RestaurantsAgent

RESTAURANT_CASE_CONCURRENCY = 3
RESTAURANT_RUN_TIMEOUT_SECONDS = 150.0
_ROOT = Path(__file__).resolve().parents[2]
BASELINE_CONFIG = get_baseline_config("restaurants")
...
def _redact_sensitive_text(text: str) -> str:
    """Remove credentials from provider URLs/errors before they reach disk."""
...
def classify_restaurant_outcome(trajectory: Trajectory) -> str:
...
async def run_restaurant_dataset(
    queries: list[str],
    *,
    model_config: dict,
    max_concurrency: int = RESTAURANT_CASE_CONCURRENCY,
    timeout: float = RESTAURANT_RUN_TIMEOUT_SECONDS,
) -> list[Trajectory]:
    """Run or reuse a pinned RestaurantsAgent snapshot for a model/dataset."""
    return await run_cached_dataset(
        config=BASELINE_CONFIG,
        queries=queries,
        model_config=model_config,
        agent_cls=RestaurantsAgent,
        classify_outcome=classify_restaurant_outcome,
