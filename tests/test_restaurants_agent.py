"""Unit tests for RestaurantsAgent — class structure and tool registration."""
...
class TestRestaurantsAgent:
    def setup_method(self):
        self.agent = RestaurantsAgent(llm=MagicMock())

    def test_extends_specialized_agent(self):
        assert isinstance(self.agent, SpecializedAgent)

    def test_name(self):
        assert self.agent.name == "RestaurantsAgent"

    def test_description_is_set(self):
        assert self.agent.description
        assert "restaurant" in self.agent.description.lower()

    def test_system_prompt(self):
        assert self.agent.system_prompt == RESTAURANTS_SYSTEM_PROMPT
    ...
    def test_repr(self):
        assert "RestaurantsAgent" in repr(self.agent)
